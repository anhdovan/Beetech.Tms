...
import beetech.app.core.network.WebRtcConector;
import beetech.app.core.utils.DataModel;
import beetech.app.core.utils.DataModelType;
...
public class WebRTCClient {
    ...
    private WebRtcConector webrtcCnnector;
    ...
    public WebRTCClient(Context context, PeerConnection.Observer observer, String username) {
        ...
        ReaderSettings settings =((BaseActivity) context).getReaderSettings();
        String serverUrl = "http://" + settings.serverIP + ":" + settings.port + "/rtc?r_id=3";
        webrtcCnnector = new WebRtcConector(WebRTCClient.this,  serverUrl);
        webrtcCnnector.connect();
        listener = new Listener() {
            @Override
            public void onTransferDataToOtherPeer(DataModel model) {
                WebRtcMesage msg = new WebRtcMesage(model.getType().name().toLowerCase(), model.getData());
                webrtcCnnector.send("SendMessage", msg);
            }
        };
        ...
        iceServer.add(PeerConnection.IceServer.builder("stun:stun3.l.google.com:19302").createIceServer());

        iceServer.add(PeerConnection.IceServer.builder("stun:stun4.l.google.com:19302").createIceServer());
    ...
    public void sendIceCandidate(IceCandidate iceCandidate, String target){
        ...
        if (listener!=null){
            listener.onTransferDataToOtherPeer(new DataModel(
                    ...
                    //target,username,iceCandidate.sdp, DataModelType.IceCandidate
            ));
        }
    }
